...
const LogContent:React.FC<LogContentProps> = (props)=>{
    ...
    useEffect(()=>{
        const timerId = window.setInterval(regularUpdate, props.refreshTimeout ?? 3000);
    ...
    const regularUpdate = ()=>{
        loadMoreLogLines(props.routeName, props.logName, loadedLineCountRef.current ?? 0)
            .then(results=>{
                setIsLoading(false);
                if(results.count>0) {
    ...
    return <div className={classes.logContainer}>
        <pre className={classes.logBlock}>
            {
                logLines.join("\n")
            }
        </pre>
